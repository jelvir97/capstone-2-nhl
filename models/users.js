...
class User {
  ...
  static async register({
    ...
    const result = await db.query(
      ...
      [googleID, firstName, lastName, email, isAdmin],
    );
  ...
  static async track(gameID, googleID, gameType) {
    if (!sportModels[gameType])
      throw new BadRequestError("Must include gameType argument");
    ...
    try {
      ...
      return {
        msg: `${result.rows[0].googleID} tracking ${result.rows[0].gameID}`,
      };
    } catch (err) {
