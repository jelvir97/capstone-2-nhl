...
class NHL_API {
    ...
    static async getSchedule(date='now'){
        const {data} = await axios.get(`https://api-web.nhle.com/v1/schedule/${date}`)
    ...
    static async getTeamStats(team){
        ...
        const {data} = await axios.get(`https://api-web.nhle.com/v1/club-stats/${team}/now`)
    ...
    static async getTeamSchedule(team, date='now'){
        if(!team) throw new BadRequestError('Must have team abbreviation')
        const {data} =  await axios.get(`https://api-web.nhle.com/v1/club-schedule/${team}/week/${date}`)
        return data;
    }
    ...
    static async getTeamRoster(team){
        if(!team) throw new BadRequestError('Must have team abbreviation')
        const {data} = await axios.get(`https://api-web.nhle.com/v1/roster/${team}/current`)
        return data;
    }
    ...
    static async getPlayByPlay(gameID){
        ...
        const {data} = await axios.get(`https://api-web.nhle.com/v1/gamecenter/${gameID}/play-by-play`)
        return data;
    }
