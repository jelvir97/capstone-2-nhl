...
describe("GET /users", () => {
  ...
  test("should fail with non-admin user", async () => {
    ...
    expect(res.body).toEqual({
      error: { message: "Must be admin", status: 401 },
    });
  });
  ...
  test("should fail with unauthenticated user", async () => {
    ...
    expect(res.statusCode).toEqual(401);
    expect(res.body).toEqual({
      error: { message: "Must be logged in", status: 401 },
    });
  });
});
...
describe("GET /users/id", () => {
  ...
  test("fails 404 not found", async () => {
    ...
    const res = await requests(app).get("/users/u3");
...
describe("POST /users/:gameType/:gameID", () => {
  ...
  test("should fail with BadRequest", async () => {
    ...
    expect(res.body).toEqual({
      error: { message: "Must include gameType argument", status: 400 },
    });
  });
